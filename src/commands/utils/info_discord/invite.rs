...
#[poise::command(
    ...
    prefix_command,
    category="Utilities",
    aliases("guildinvite", "invitelookup"),
    ...
    let embed = match ctx.http().get_invite(invite.as_str(), true, true, None).await {
        Ok(inv) => {
            ...
            if let Some(guild) = inv.guild {
                embed = embed.clone().description(format!("**{}**\n{}", guild.name, guild.clone().description.unwrap_or("No description".to_owned())));
                ...
                if let Some(banner) = guild.banner {
                    embed = embed.clone().image(format!("https://cdn.discordapp.com/banners/{}/{}.png", guild.id, banner.to_string()))
                } else if let Some(splash) = guild.splash_url() {
                    embed = embed.clone().image(splash)
                }
